...
```
PHASE 1 — LOCAL BACKEND (Weeks 1–6.5)
────────────────────────────────────────
...
**Tests:** 29 new auth/invite/link-slack tests. Total: 264 passing.

---

### ✅ Phase 1 Backend Complete — Ready for AWS Deployment
...
| 9 | Person knowledge graph — graph API endpoint + frontend rendering | `feature/week9-person-graph` |
| 10 | Org calibration — recompile from ExtractionFeedback signals weekly | `feature/week10-calibration` |
| 11 | Teams + Google Meet connectors | `feature/week11-connectors` |
...
COMMITMENTS
  GET    /commitments/              List with filters:
                                    ?status= ?owner= ?deadline_before= ?risk_gte= ?source= ?tags__label=
...
Phase 1 backend is complete — 264 tests passing. Next step: AWS deployment.*
